...
// ── Close all on Escape ──
document.addEventListener('keydown', e => { if (e.key === 'Escape') { closeModal(); closePdfModal(); closeScicommModal(); } });
...
contactForm.addEventListener('submit', async e => {
  ...
  contactForm.querySelectorAll('.fg').forEach(fg => fg.classList.remove('has-error'));
  ...
  if (!nameEl.value.trim()) {
    nameEl.closest('.fg').classList.add('has-error'); valid = false;
  }
  const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!emailEl.value.trim() || !emailPattern.test(emailEl.value.trim())) {
    emailEl.closest('.fg').classList.add('has-error'); valid = false;
  }
  if (!messageEl.value.trim()) {
    messageEl.closest('.fg').classList.add('has-error'); valid = false;
  }
  if (!valid) return;
  ...
  try {
    ...
    if (res.ok) {
      ...
      alert('Something went wrong. Please try emailing directly at garima99nitrkl@gmail.com');
    }
  } catch {
    sendBtn.disabled = false;
    sendBtn.textContent = 'Send Message ↗';
    alert('Network error. Please try emailing directly at garima99nitrkl@gmail.com');
  }
});
